...
export const GRIDS_RULES = [
    ...
    // Rule 2: Symmetric (vertical OR horizontal)
    {
        ...
        distribution: {
            positive: [
                { type: 'anySymmetry', weight: 0.8 },
                { type: 'fullySymmetric', weight: 0.2 },
            ]
        },
    ...
    {
        id: 'g4',
        name: 'Kaikki mustat ovat reunalla',
        check: (grid) => {
            for (let i = 0; i < 6; i++) {
                for (let j = 0; j < 6; j++) {
                    if (grid[i][j]) {
    ...
    {
        id: 'g5',
        name: 'Jokaisella mustalla ruudulla on vähintään yksi naapuri',
        check: (grid) => {
            for (let i = 0; i < 6; i++) {
                for (let j = 0; j < 6; j++) {
                    if (grid[i][j]) {
        ...
        distribution: {
            positive: [
                { type: 'many2_4s', weight: 0.15},
                { type: 'twoLargeComponents', weight: 0.2},
                { type: 'threeMediumComponents', weight: 0.2},
                { type: 'fourSquiqqles', weight: 0.15},
                { type: 'many3s', weight: 0.15},
                { type: 'longSquiqqle', weight: 0.15},
            ]
        },
        roundSpecificNegative: [
    ...
    {
        id: 'g10',
        name: 'Jollakin mustalla on 4 naapuria',
        check: (grid) => {
            for (let i = 0; i < 6; i++) {
                for (let j = 0; j < 6; j++) {
                    if (grid[i][j]) {
