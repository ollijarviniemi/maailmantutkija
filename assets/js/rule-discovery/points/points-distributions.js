...
// Validation function - throws exception if point is outside valid range
function validatePoint(point) {
    if (point.x < MIN_X || point.x > MAX_X || point.y < MIN_Y || point.y > MAX_Y) {
        throw new Error(`Point (${point.x}, ${point.y}) is outside valid range [${MIN_X}, ${MAX_X}] × [${MIN_Y}, ${MAX_Y}]`);
    }
}

// Validation function for arrays of points
...
export function almostCollinear() {
    const line = anyLine();
    if (line.length < 3) return line;

    // Move one point slightly off the line
    const idx = randomInt(1, line.length - 1);
    const moved = { ...line[idx] };
    moved[randomChoice(['x', 'y'])] += randomChoice([-1, 1]);
...
const BASELINE_DISTRIBUTION_RAW = [
    ...
    { type: 'almostCollinear', weight: 0.01 },
