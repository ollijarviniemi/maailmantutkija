...
export const POINTS_RULES = [
    ...
    {
        id: 'p2',
        name: 'Kaikilla pisteillä on eri x-koordinaatti ja eri y-koordinaatti',
        check: (points) => {
    ...
    // Rule 5: Some three points are collinear
    {
        ...
        roundSpecificNegative: [
            ...
            { type: 'almostCollinear', weight: 0.3 },
    ...
    // Rule 10: No unique nearest neighbor
    {
        id: 'p10',
        name: 'Jokaisen kaksi lähintä naapuria ovat yhtä kaukana',
        check: (points) => {
            if (points.length <= 1) return true;
        ...
        distribution: {
            positive: [
                { type: 'p10RotatedSquare', weight: 0.1 },
                { type: 'p10OverlappingUnitSquares', weight: 0.1 },
                { type: 'p10SquareWithBisectorPoints', weight: 0.15 },
                { type: 'p10LoopPatterns', weight: 0.15 },
                { type: 'p10LoopWithBisectorPoints', weight: 0.15 },
                { type: 'p10TwoSquaresPlusNinth', weight: 0.10 },
                { type: 'p10TreeShapes', weight: 0.25 }
            ]
        },
        roundSpecificNegative: [
            { type: 'randomScattered', weight: 0.4 },
            { type: 'generalPosition', weight: 0.3 },
            { type: 'triangle', weight: 0.3 }
        ],
        generateNegative: () => {
            // Points where at least one has unique nearest neighbor
            return [
                { x: 5, y: 5 },
                { x: 8, y: 5 },
                { x: 13, y: 10 }
