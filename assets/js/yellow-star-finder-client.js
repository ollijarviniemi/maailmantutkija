/**
 * Client-side Yellow Star Finder App
 * EXACT replication of the Flask backend yellow-star-finder.js
...
class YellowStarFinderApp {
    ...
    initializeApp() {
        ...
        console.log('Yellow Star Finder App initialized');
    }
    ...
    loadSavedData() {
        try {
            // FIXED: Use correct localStorage key from old implementation
    ...
    // FIXED: Restore exact variance reduction system from old implementation
    ...
    async startNewRound() {
        ...
        try {
            ...
            const imageData = await this.generateImageWithTarget(targetHasStar);

            if (imageData && imageData.success) {
                // Store the correct answer and image data
                this.hasTarget = imageData.hasTarget;
                this.currentImageData = {
                    imageData: imageData.image,
                    hasTarget: imageData.hasTarget,
                    starPositions: imageData.starPositions || []
                };

                // Display the image
                await this.displayBase64Image(imageData.image);

                console.log(`Generated new image (target: ${imageData.hasTarget})`);
            }
    ...
    async generateImageWithTarget(shouldHaveTarget) {
        // Create a client-side shape finder if it doesn't exist
        if (!this.shapeFinder) {
            this.shapeFinder = new ClientShapeFinder(800, 600);
        }

        // Generate image with specific target requirement - use 150 shapes like original
        const numShapes = 150;

        return await this.shapeFinder.generateShapeFinderImage({
            targetShape: "star",
            targetColor: "yellow",
            numShapes: numShapes,
            hasTarget: shouldHaveTarget,
    ...
    updateRoundCounter() {
        ...
        this.roundCounter.style.display = 'block';
    ...
    showImageWithTimer() {
        ...
        // Don't show timer display container - we only use the progress bar now
        // this.timerDisplay.style.display = 'flex';

        // Don't show the numerical countdown - only the progress bar
        // this.timerCountdown.textContent = this.selectedTime;
        // this.timerText.classList.add('visible');

        // Show and animate progress bar (vertical)
        this.progressBarContainer.classList.add('visible');
        // Reset progress bar to 100% height with no transition
        this.progressBarFill.style.transitionDuration = '0s';
        this.progressBarFill.style.height = '100%';

        // Force reflow to ensure the reset is applied before animation
        void this.progressBarFill.offsetHeight;

        // Start smooth transition to 0% over the selected time
        this.progressBarFill.style.transitionDuration = this.selectedTime + 's';
    ...
    // FIXED: Restore exact countdown timer logic from old implementation
    startCountdownTimer() {
    ...
    // FIXED: Restore exact image hiding logic from old implementation
    hideImageAndStartQuestion() {
        ...
        // Hide timer display and progress bar
        this.timerDisplay.style.display = 'none';
        this.timerText.classList.remove('visible');
        this.progressBarContainer.classList.remove('visible');
    ...
    // FIXED: Use boolean response values like old implementation
    ...
    drawRedCirclesAroundYellowStars() {
        ...
        this.currentImageData.starPositions.forEach(star => {
            ...
            // FIXED: No Y-axis flip needed since client-side generator already uses Canvas coordinate system
            const canvasY = star.y * scaleY;
    ...
    async startBuffering() {
        ...
        try {
            const imageData = await this.generateImageWithTarget(Math.random() < 0.67);
            ...
            if (imageData) {
                ...
                console.log(`Buffered next image (target: ${imageData.hasTarget})`);
            }
        } catch (error) {
    ...
    // FIXED: Restore exact statistics display from old implementation
    showStatistics() {
        ...
        this.roundCounter.style.display = 'none';
        ...
        // Create statistics HTML with left-aligned text - EXACT from old implementation
        let statsHtml = `
    ...
    // FIXED: Restore exact statistics calculation from old implementation
    calculateStatistics() {
...
function initializeYellowStarFinderApp() {
    console.log('Initializing YellowStarFinderApp...');
    try {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initializeYellowStarFinderApp);
} else {
    // DOM already loaded (script loaded dynamically after page load)
    initializeYellowStarFinderApp();
}