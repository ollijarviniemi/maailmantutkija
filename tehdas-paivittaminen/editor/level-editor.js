...
class LevelEditor {
  constructor() {
    ...
    this.definedSacks = {}; // varName -> {id, distribution, varName}
    this.definedArms = {}; // varName -> {id, varName}
    this.placedSacks = new Set(); // Track which sacks have been placed
    this.placedArms = new Set(); // Track which arms have been placed
  ...
  onKeyDown(e) {
    ...
    if (!ctrl && !e.shiftKey) {
      ...
      // Component shortcuts - auto-place next unplaced sack/arm
      else if (e.key === 's' || e.key === 'S') {
        this.autoPlaceNextSack();
        e.preventDefault();
  ...
  placeComponent(x, y) {
    ...
    // Handle sack placement
    if (component.type === 'sack') {
      const varName = this.pendingComponentParams.varName;

      if (varName && this.definedSacks[varName]) {
        // This is a DSL-defined sack
        component.params.varName = varName;
        component.params.distribution = this.definedSacks[varName].distribution;
        component.params.label = varName; // Display variable name
        this.placedSacks.add(varName);

        // Check if this was the last sack - if so, auto-deselect
        if (this.placedSacks.size === Object.keys(this.definedSacks).length) {
          this.selectedComponentType = null;
          this.pendingComponentParams = {};
        } else {
          // Auto-select next sack
          this.autoPlaceNextSack();
        }
      } else if (this.selectedTemplate && this.sackTemplates[this.selectedTemplate]) {
        // Template-based sack (List/export system)
        const templateId = this.selectedTemplate;
        component.params.linkedTemplate = templateId;
        component.params.label = templateId;
        component.params.varName = templateId; // Use templateId as varName for tracking
        this.placedSacks.add(templateId);

        // Check if this was the last template sack - if so, auto-deselect
        if (this.placedSacks.size === Object.keys(this.sackTemplates).length) {
          this.selectedComponentType = null;
          this.selectedTemplate = null;
          this.pendingComponentParams = {};
        } else {
        ...
        throw new Error('Trying to place ambiguous sack!');
      }
    }

    // Handle arm placement
    if (component.type === 'arm') {
      const varName = this.pendingComponentParams.varName;

      if (varName && this.definedArms[varName]) {
        ...
        if (this.placedArms.size === Object.keys(this.definedArms).length) {
          this.selectedComponentType = null;
          this.pendingComponentParams = {};
        } else {
  ...
  deleteComponent(component) {
    ...
    if (index > -1) {
      ...
      // If this was a DSL-defined sack/arm, mark it as unplaced
      if (component.type === 'sack' && component.params.varName) {
        this.placedSacks.delete(component.params.varName);
      } else if (component.type === 'arm' && component.params.varName) {
  ...
  rotateComponent(component) {
    ...
    if (component.type === 'conveyor') {
      ...
      component.params.sides = rotateSidesClockwise(component.params.sides || defaultDuplicatorSides(component.params));
      delete component.params.outputPattern;  // legacy field, no longer used
    } else if (component.type === 'merger') {
      // Rotate merger through configurations (includes 3-input mergers)
      ...
      if (spec && spec.getNextRotation) {
        ...
        component.params.input3Side = nextConfig.input3Side || null;
      ...
      // Re-run auto-detection when rotation key is pressed
      this.autoDetectSplitterChannels(component);
    } else if (component.type === 'shuffler') {
      component.params.sides = rotateSidesClockwise(component.params.sides || defaultShufflerSides(component.params));
      delete component.params.outputPattern;  // legacy field, no longer used
    }
  ...
  rotatePendingComponent() {
    ...
    if (type === 'conveyor') {
      ...
      if (spec && spec.getNextRotation) {
        ...
        this.pendingComponentParams.input3Side = nextConfig.input3Side || null;
      ...
      this.pendingComponentParams.sides = rotateSidesClockwise(this.pendingComponentParams.sides || defaultDuplicatorSides(this.pendingComponentParams));
      delete this.pendingComponentParams.outputPattern;
    } else if (type === 'shuffler') {
      this.pendingComponentParams.sides = rotateSidesClockwise(this.pendingComponentParams.sides || defaultShufflerSides(this.pendingComponentParams));
      delete this.pendingComponentParams.outputPattern;
    }
  ...
  editOutputPattern() {
    ...
    if (patternStr !== null) {
      try {
        ...
        delete component.params.outputPattern;
        this.updateAutomaticConnections();
        this.updatePropertiesPanel();
        this.saveState();
        this.render();
  ...
  loadLevel() {
    ...
    if (level.hypothesisScript) {
      ...
      try {
        ...
        // After executing, track which sacks/arms are already placed
        this.placedSacks = new Set();
        this.placedArms = new Set();

        this.components.forEach(comp => {
          if (comp.type === 'sack' && comp.params.varName) {
            this.placedSacks.add(comp.params.varName);
          } else if (comp.type === 'arm' && comp.params.varName) {
  ...
  saveLevel() {
    // Validate that sampling schedule exists
    if (!this.samplingSchedule || this.samplingSchedule.length === 0) {
      this.setStatus('Error: Sampling schedule must be defined in hypothesis script', 'error');
      alert('Error: Sampling schedule must be defined using schedule() function in hypothesis script');
      return;
    }

    // Get existing levels from LevelRegistry (disk-backed).
    const savedLevels = this.getSavedLevels();

    // Find maximum existing level number to avoid ID collisions
    // (Display number is separate and based on array position)
    ...
    // Title and description are just defaults (user sees array position as number)
    ...
    const level = {
      ...
      simulation: {
        ...
        ballSpeed: 1.0,
        ballsToSpawn: this.samplingSchedule.length,  // Determined by schedule
        seed: 42
      }
    };

    // Save to LevelRegistry (disk-backed via /save-export/<game>).
  ...
  loadLevelById(levelId) {
    ...
    // Load level data (same as loadLevel() method)
    this.gridWidth = level.grid.width;
    this.gridHeight = level.grid.height;
    this.components = level.components || [];
    this.connections = level.connections || [];
  ...
  drawComponent(component) {
    ...
    if (component.params?.varName) {
      ...
      this.ctx.fillText(`(${component.id})`, px + 2, py + 14);
    }
  }
  ...
  updateAutomaticConnections() {
    ...
    this.components.forEach(comp => {
      ...
      if (comp.type === 'sack') {
        ...
        // Arm outputs to any component that can receive balls
        const output = adjacent.find(c => this.canReceiveBalls(c.type));
        if (output) {
  ...
  executeHypothesisScript() {
    ...
    // Store sacks and arms from DSL
    this.definedSacks = result.sacks || {};
    this.definedArms = result.arms || {};

    // Track which sacks/arms have been placed
    this.placedSacks = new Set();
    this.placedArms = new Set();
    ...
    // Update sack/arm palettes
    this.updateSackArmPalettes();

    const sackCount = Object.keys(this.definedSacks).length;
    const armCount = Object.keys(this.definedArms).length;
    this.setStatus(`Created ${sackCount} sack(s), ${armCount} arm(s), ${Object.keys(result.templates).length} template(s)`, 'success');
  }
  ...
  updateSackArmPalettes() {
    // This method can be used to update UI displays of sacks/arms
    // For now, we're handling placement directly via hotkeys
    console.log('Defined sacks:', Object.keys(this.definedSacks));
    console.log('Defined arms:', Object.keys(this.definedArms));
  }

  autoPlaceNextSack() {
    // Check if using DSL sack() system or template system
    const availableSacks = Object.keys(this.definedSacks).length > 0
      ? this.definedSacks
      : this.sackTemplates;

    // Find the first unplaced sack
    for (const varName of Object.keys(availableSacks)) {
      if (!this.placedSacks.has(varName)) {
        // Select this sack for placement
        this.currentTool = 'place';
        this.selectedComponentType = 'sack';

        if (this.definedSacks[varName]) {
          // DSL sack() system
          this.pendingComponentParams = {
            varName: varName,
            id: this.definedSacks[varName].id,
            distribution: this.definedSacks[varName].distribution
          };
        } else {
          // Template system - use existing selectTemplate logic
          this.selectedTemplate = varName;
          this.pendingComponentParams = {};
        }

        this.setStatus(`Ready to place sack: ${varName}`, 'info');
        this.render();
        return;
      }
    }

    // All sacks placed - do nothing
    this.setStatus('All sacks already placed', 'info');
  }

  autoPlaceNextArm() {
    // Find the first unplaced arm
    for (const varName of Object.keys(this.definedArms)) {
      if (!this.placedArms.has(varName)) {
        ...
        this.pendingComponentParams = {
          varName: varName,
          id: this.definedArms[varName].id
        };
