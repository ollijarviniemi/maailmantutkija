...
class DSLEditor {
  ...
  showResult(distribution) {
    ...
    // Render distribution as bar chart. Hide outcomes with prob < 0.1%.
    const MIN_VISIBLE_PROB = 0.001;
    const allEntries = distribution.entries();
    const entries = allEntries.filter(([, p]) => p >= MIN_VISIBLE_PROB);
    const hiddenCount = allEntries.length - entries.length;
    let hiddenSum = 0;
    for (const [, p] of allEntries) if (p < MIN_VISIBLE_PROB) hiddenSum += p;
    const maxProb = entries.length > 0 ? Math.max(...entries.map(([_, p]) => p)) : 1;
    ...
    if (hiddenCount > 0) {
      const pct = (hiddenSum * 100).toFixed(1);
      const prefix = hiddenSum > 0 && pct === '0.0' ? '~' : '';
      html += `<div class="result-more">... ja ${hiddenCount} muuta (yht. ${prefix}${pct}%)</div>`;
    }
    html += '</div>';
  ...
  onMouseMove(e) {
    ...
    const info = this.getWordAndLineAtPosition(e);
    const word = info ? info.word : null;
    const line = info ? info.line : null;
    ...
    if (word && this.lastResult.variables[word]) {
      // Use per-line distribution: show value as of the hovered line
      const dist = (line != null && this.interpreter.getVariableDistributionAtLine)
        ? this.interpreter.getVariableDistributionAtLine(word, line)
        : this.interpreter.getVariableDistribution(word);
      if (dist) {
  ...
  /**
   * Get word at cursor position in textarea (backward-compat wrapper)
   */
  getWordAtPosition(event) {
    const r = this.getWordAndLineAtPosition(event);
    return r ? r.word : null;
  }

  /**
   * Get word and 1-based line number at cursor position in textarea
   */
  getWordAndLineAtPosition(event) {
    ...
    return { word: line.slice(start, end), line: lineIndex + 1 };
  }
  ...
  showTooltip(varName, distribution, x, y) {
    ...
    if (allEntries.length > 0) {
      const firstValue = allEntries[0][0];
    ...
    // Filter out outcomes below 0.1% — they're noise / rounding tails that
    // just clutter the tooltip.
    const MIN_VISIBLE_PROB = 0.001;
    const visibleEntries = allEntries.filter(([, p]) => p >= MIN_VISIBLE_PROB);
    ...
    // Extra rows: those beyond the first 10, plus the <0.1% tail
    const overflowFromTop = Math.max(0, entries.length - 10);
    let overflowSum = 0;
    for (const [, p] of entries.slice(10)) overflowSum += p;
    overflowSum += hiddenSum;
    const extra = overflowFromTop + hiddenCount;
    if (extra > 0) {
      const pct = (overflowSum * 100).toFixed(1);
      const prefix = overflowSum > 0 && pct === '0.0' ? '~' : '';
      html += `<div class="tooltip-more">... ja ${extra} muuta (yht. ${prefix}${pct}%)</div>`;
    }
