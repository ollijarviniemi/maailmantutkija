/**
 * Level Editor for Todennakoisyysmallit
 *
...
// Build a default sides config for a duplicator (one output side, default 'right').
...
// Build a default sides config for a shuffler.
function defaultShufflerSides(params) {
  ...
  // Default input on opposite side
  const opposite = {up: 'down', right: 'left', down: 'up', left: 'right'};
  sides[opposite[outputSide]] = {type: 'input', count: 0};
  return sides;
}

// Convert a sides config to the legacy pattern array (for editor display/edit).
...
// Convert an edited pattern array back into a sides config, preserving inputs.
...
class LevelEditor {
   ...
   */
  computeDistributionBranching(simLevel, prediction, targetId, observationPoints, maxBranches = 50000) {
    const config = {
    ...
    while (queue.length > 0 && branchCount < BFS_BUDGET) {
      ...
      while ((sim.running || hasActiveBalls(sim)) && !decisionFound && ticks < MAX_TICKS_PER_BRANCH) {
        ...
        if (branchingRng.hasPendingDecision()) {
          decisionFound = true;
          const decisions = branchingRng.consumeAllPendingDecisions();
          // Cartesian product: multiple simultaneous choices in one tick
          // (e.g., duplicator output flooding a splitter).
          const cartesian = [[]];
          ...
          for (const combo of cartesian) {
            ...
            branchingRng.forcedOutcomes = combo.map(o => o.value);
            sim.tick(TICK_DT);
            if (branchingRng.forcedOutcomes.length > 0) {
              throw new Error(`BranchingRNG: replayed tick consumed fewer decisions than original (${branchingRng.forcedOutcomes.length} unused)`);
            }
            if (branchingRng.hasPendingDecision()) {
              throw new Error(`BranchingRNG: replayed tick produced new decisions not seen in original (${branchingRng.pendingDecisions.length} extra)`);
            }
            let pCombo = 1;
            for (const o of combo) pCombo *= o.probability;
            queue.push({
              simState: serializeSimState(sim),
              probability: probability * pCombo
            });
          }
        }
      }
  ...
  testLevel() {
    ...
    console.log('[Editor testLevel] Components:', this.components.length, 'Buttons:', this.components.filter(c => c.type === 'button').length);
    console.log('[Editor testLevel] Button components:', this.components.filter(c => c.type === 'button'));
    ...
    localStorage.setItem('tehdas-simulointi_testLevel', JSON.stringify(level));
...
/**
 * Recompute all level distributions using branching + MC.
...
function recomputeAllDistributions() {
  ...
  for (const level of allLevels) {
    if (!level.dgpResult || !level.dgpResult.prediction) continue;
    ...
    try {
      ...
      if (prediction.what === 'dist') {
        observationPoints = tmpEditor.components
          .filter(c => c.type === 'observation')
          .sort((a, b) => a.position.x - b.position.x)
          .map((comp, index) => ({
        ...
        const targetLabel = prediction.target;
        targetComponent = tmpEditor.components.find(c =>
          c.type === 'observation' &&
          (c.params?.label === targetLabel || c.id === targetLabel)
        );
      }
      ...
      level.correctDistribution = dist;
      updated++;
      console.log(`✓ ${level.meta.title}: ${JSON.stringify(dist)}`);
    } catch (e) {
      errors++;
      console.error(`✗ ${level.meta.title}: ${e.message}`);
    }
  }
